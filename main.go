...
func messageCreate(s *discordgo.Session, m *discordgo.MessageCreate) {
	...
	if len(switchCon) >= 1 {
		...
		case "!card":
			{
				if IsNumeric(switchCon[len(switchCon)-1]) {
					count, _ := strconv.Atoi(switchCon[len(switchCon)-1])
					if count > 5 {
						response := ("Please enter number between 2 & 5, for single cards dont include a number")
						_, _ = s.ChannelMessageSend(m.ChannelID, response)
					}
					if count <= 5 {
						response := functions.CardCounter(msg, count)
						_, _ = s.ChannelMessageSend(m.ChannelID, response)
					}
				} else {
					response := functions.CardCounter(msg, 1)
					_, _ = s.ChannelMessageSend(m.ChannelID, response)
				}
			}
		case "!gobot":
...
//IsNumeric is used for number parse
